...
protein.clean <- read_csv("../MM_ProteinCleaned.csv")
...
tuneplot <- function(x, probs = .90) {
  ggplot(x) +
    coord_cartesian(ylim = c(min(x$results$F1), max(x$results$F1))) +
    theme_bw()
}

tuneplot(xgb_tune)
xgb_tune$bestTune
max(xgb_tune$results$F1)
...
write_csv(x=xgb.preds, path="./ES_XGBPredictions_F1.csv")
